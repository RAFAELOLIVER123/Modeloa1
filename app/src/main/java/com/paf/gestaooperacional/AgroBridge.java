...
public class AgroBridge {
    ...
    @JavascriptInterface public String getState(){
        try{
            ...
            o.put("authenticated",!db.getSetting("token","").isEmpty());
    ...
    @JavascriptInterface public void login(String login,String password){
        new Thread(()->{
            try{
                ...
                SyncEngine.refresh(activity,r.getInt("group_id"));
                callback("agroLoginResult",ok("Acesso realizado. A base foi baixada para o aparelho."));
    ...
    @JavascriptInterface public void refreshData(int groupId){
        new Thread(()->{
            try{
                if(!ApiClient.isOnline(activity))throw new IllegalStateException("Sem internet. Continue usando os dados que já estão salvos neste aparelho.");
                SyncEngine.refresh(activity,groupId);
                callback("agroRefreshResult",ok("Base completa atualizada no aparelho."));
    ...
    @JavascriptInterface public void syncNow(){
        new Thread(()->{
            try{
                if(!ApiClient.isOnline(activity))throw new IllegalStateException("Sem internet. Nada foi perdido: os registros continuam na memória do aparelho.");
    ...
    @JavascriptInterface public String setGroup(int groupId){
        try{
            ...
            return ok("Grupo alterado. Atualize a base quando estiver com internet.");
        }catch(Exception e){return fail(message(e));}
    }
    ...
    @JavascriptInterface public void logout(){
        ...
        db.logoutKeepData();
        if(!token.isEmpty()&&ApiClient.isOnline(activity))new Thread(()->{try{ApiClient.logout(token);}catch(Exception ignored){}}).start();
        callback("agroLoggedOut",ok("Sessão encerrada. A base baixada permanece no aparelho."));
    }

    @JavascriptInterface public String clearLocalData(){
        try{db.clearEverything();return ok("Dados locais apagados.");}catch(Exception e){return fail(message(e));}
    }
    ...
    private static String message(Exception e){String m=e.getMessage();return m==null||m.isEmpty()?"Ocorreu uma falha no aplicativo.":m;}
