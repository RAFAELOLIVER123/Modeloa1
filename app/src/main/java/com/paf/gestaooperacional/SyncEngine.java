...
public class SyncEngine {
    ...
    public static JSONObject refresh(Context ctx,int requestedGroup) throws Exception {
        ...
        if(gid<=0)gid=1;
        JSONObject j=ApiClient.bootstrap(token,gid);
    ...
    public static JSONObject syncAll(Context ctx,Progress progress,boolean refreshAfter) throws Exception {
        ...
        int totalInicial=db.pendingCount();
        int done=0,failed=0,processed=0;
        final int LIMITE_SEGURANCA=5000;

        while(processed<LIMITE_SEGURANCA){
            JSONArray pending=db.pending(300);
            if(pending.length()==0)break;
            boolean houveSucesso=false;

            for(int i=0;i<pending.length() && processed<LIMITE_SEGURANCA;i++){
                JSONObject q=pending.getJSONObject(i);
        ...
        if(refreshAfter && db.pendingCount()==0){
            try{fresh=refresh(ctx,parseInt(db.getSetting("group_id","0")));}catch(Exception ignored){}
        }
        JSONObject out=new JSONObject();
        out.put("ok",db.pendingCount()==0);
        out.put("sincronizados",done);
        out.put("falhas",failed);
        out.put("pendentes",db.pendingCount());
        out.put("synced",done);
        out.put("failed",failed);
        out.put("pending",db.pendingCount());
        if(processed>=LIMITE_SEGURANCA)out.put("aviso","A fila é muito grande. Sincronize novamente para continuar.");
        if(fresh!=null)out.put("base_atualizada",true);
        return out;
    }
